...
@interface MKLinkMapVC() <NSTableViewDelegate, NSTableViewDataSource, NSDraggingDestination>
...
@property (nonatomic, strong) MKDSYMVC *MKDSYMVC;
@end
...
#pragma mark - toolbar
#pragma mark  切换工具
- (IBAction)dsym:(id)sender {
    self.MKDSYMVC = [[MKDSYMVC alloc] initWithWindowNibName:@"MKDSYMVC"]; 
    [self.MKDSYMVC showWindow:self];
}
...
- (IBAction)print:(id)sender {
    if (!_linkMapFileURL || ![[NSFileManager defaultManager] fileExistsAtPath:[_linkMapFileURL path] isDirectory:nil]) {
        [self alert:@"请选择正确的 Link Map 文件路径"];
        return;
    }
    [self startAnimation];
